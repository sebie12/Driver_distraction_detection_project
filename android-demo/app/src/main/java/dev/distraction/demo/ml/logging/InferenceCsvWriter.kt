package dev.distraction.demo.ml.logging
...
class InferenceCsvWriter(
    ...
    fun open(sessionFolder: File) {
        ...
        if (!fileExists || csvFile!!.length() == 0L) {
            writer?.append(
                "window_start_ms,window_end_ms,model_id,model_version,score,predicted_class,threshold,sensor_samples_count,location_samples_count,device_samples_count,extras\n"
            )
            writer?.flush()
        }
    }
    ...
    fun write(
        ...
        val extrasString = extras.entries.joinToString(";") { (k, v) ->
            "${escape(k)}=${escape(v)}"
        }
    ...
    companion object {
        private const val FILE_NAME = "inference.csv"
    }
}